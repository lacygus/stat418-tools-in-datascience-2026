...
class TMDBCollector:
    """Collect popular movie data, details, and credits from the TMDB API."""

    BASE = "https://api.themoviedb.org/3"
    INTERVAL = 0.25

    def __init__(self) -> None:
        """Initialize the API session and read the TMDB API key from the environment."""
    ...
    def _get(self, path: str, params: Dict | None = None) -> Dict:
        """Make a rate-limited TMDB GET request with basic retry handling."""
        ...
        for attempt in range(3):
            try:
                ...
                logging.warning("GET %s failed on attempt %d: %s", path, attempt + 1, e)
                if attempt == 2:
                    raise
    ...
    def get_movie_details(self, movie_id: int) -> Dict:
        """Fetch detailed metadata for a single movie."""
    ...
    def collect_all_data(self, num_items: int = 50) -> List[Dict]:
        """Collect detailed TMDB records for the requested number of popular movies."""
        movies: List[Dict] = []
        page = 1
...
def main() -> None:
    """Run TMDB collection and save raw movie records to JSON."""
    os.makedirs("data/raw/tmdb", exist_ok=True)
    c = TMDBCollector()
    print("Collecting TMDB data...")
    data = c.collect_all_data(50)
    out = "data/raw/tmdb/movies.json"
    with open(out, "w") as f:
        json.dump(data, f, indent=2)
    print(f"Saved {len(data)} movies → {out}")
    logging.info("Done: %d movies saved", len(data))
